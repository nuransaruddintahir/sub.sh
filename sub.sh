...
#-Metadata----------------------------------------------------#
#  Filename: sub.sh (v1.0.21-mod-macos)                      #
#-Info--------------------------------------------------------#
# Subdomain Detect Script (macOS-friendly installer)         #
#-------------------------------------------------------------#
...
function 17amass() {
    amass enum -passive -d "$1" -o "amass_$1.txt" 2> "amass_$1.err"
...
function commonToolInstall(){
    ...
    # Ensure ~/go/bin is set up and in PATH for the session
    ...
    # Install go if missing (use brew on macOS)
    if ! command -v go >/dev/null 2>&1; then
        if command -v brew >/dev/null 2>&1; then
            echo -e "${BLUE}[!] Installing go via brew ...${RESET}"
            brew install go
        else
            echo -e "${RED}[!] go not found and brew not available. Please install Go manually.${RESET}"
            return 1
        fi
    fi

    # httprobe
    if command -v httprobe >/dev/null 2>&1 || [ -e "${GOBIN}/httprobe" ]; then
        echo -e "${BLUE}[!] httprobe already exists \n${RESET}"
    else
        echo -e "${BLUE}[!] Installing httprobe via go install ...\n${RESET}"
        GO111MODULE=on go install github.com/tomnomnom/httprobe@latest
        echo -e "${RED}[!] httprobe installed \n${RESET}"
    fi

    # subfinder (v2)
    if command -v subfinder >/dev/null 2>&1 || [ -e "${GOBIN}/subfinder" ]; then
        echo -e "${BLUE}[!] Subfinder already exists \n${RESET}"
    else
        echo -e "${BLUE}[!] Installing subfinder via go install ...\n${RESET}"
        GO111MODULE=on go install -v github.com/projectdiscovery/subfinder/v2/cmd/subfinder@latest
        echo -e "${RED}[!] Subfinder installed \n${RESET}"
    fi

    # assetfinder
    if command -v assetfinder >/dev/null 2>&1 || [ -e "${GOBIN}/assetfinder" ]; then
        echo -e "${BLUE}[!] Assetfinder already exists \n${RESET}"
    else
        echo -e "${BLUE}[!] Installing assetfinder via go install ...\n${RESET}"
        GO111MODULE=on go install github.com/tomnomnom/assetfinder@latest
        echo -e "${RED}[!] Assetfinder installed \n${RESET}"
    fi

    # findomain handled in installOSX; for other OS we keep original logic
    # amass handled in installOSX via brew
...
function installOSX(){
    echo -e "${BLUE}[*] Running macOS installer using Homebrew ...${RESET}"
    brew update

    # basic packages
    brew install jq
    brew install findomain || true   # homebrew may have formula or tap; ignore failure
    brew tap caffix/amass 2>/dev/null || true
    brew install amass || true
    brew install parallel || true

    # ensure go is installed for go-based tools
    if ! command -v go >/dev/null 2>&1; then
        echo -e "${BLUE}[!] Installing go via brew ...${RESET}"
        brew install go
    fi

    # make sure GOPATH/GOBIN exist and in PATH for current session
    export GOPATH="${HOME}/go"
    export GOBIN="${GOPATH}/bin"
    mkdir -p "${GOBIN}"
    if ! echo "$PATH" | grep -q "${GOBIN}"; then
        export PATH="${GOBIN}:$PATH"
        echo "export GOPATH=\"${GOPATH}\"" >> ~/.bash_profile 2>/dev/null || true
        echo "export PATH=\"${GOBIN}:\$PATH\"" >> ~/.bash_profile 2>/dev/null || true
        # also add to zsh if present
        echo "export GOPATH=\"${GOPATH}\"" >> ~/.zprofile 2>/dev/null || true
        echo "export PATH=\"${GOBIN}:\$PATH\"" >> ~/.zprofile 2>/dev/null || true
    fi

    echo -e "${GREEN}[!] MAC-OSX Tool Installed (brew packages). Now installing Go-based tools... \n${RESET}"
    commonToolInstall
    ...
    echo -e "${BLUE}[!] Note: make sure ${GOBIN} is in your PATH (restart shell or source ~/.bash_profile / ~/.zprofile)${RESET}"
