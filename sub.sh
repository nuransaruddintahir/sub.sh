...
#-Metadata----------------------------------------------------#  
#  Filename: sub.sh (v1.0.23)    (Update: 2026-07-20)         #  
#-Info--------------------------------------------------------#  
...
function installGo() {  
    if command -v go &> /dev/null; then  
        echo -e "${BLUE}[!] Go is already installed: $(go version)${RESET}"  
        return 0  
    fi  
      
    echo -e "${RED}[!] Installing Go via apt...${RESET}"  
    ...
    echo -e "${GREEN}[!] Go installation complete!${RESET}"  
}
...
function 7anubisdb(){  
	...
	echo "[+] Anubis-DB Over => $(wc -l anubisdb_$1.txt | awk '{ print $1}')"  
}  
...
function 9alienvault(){  
	...
	echo "[+] Alienvault Over => $(wc -l alienvault_$1.txt | awk '{ print $1}')"  
}  
function 10urlscan(){  
...
function 14dnsdumpster() {  
	...
	curl -s --header "Host:dnsdumpster.com" --referer https://dnsdumpster.com --user-agent "Mozilla/5.0" --data "csrfmiddlewaretoken=$cmdtoken&targetip=$1" --cookie "csrftoken=$cmdtoken" https://dnsdumpster.com > dnsdumpster.html
...
function commonToolInstall(){
	...
	if [ -e ~/go/bin/httprobe ] || [ -e /usr/local/bin/httprobe ] || command -v httprobe &> /dev/null; then  
	...
	if [ -e ~/go/bin/subfinder ] || [ -e /usr/local/bin/subfinder ] || command -v subfinder &> /dev/null; then  
	...
	if [ -e ~/go/bin/assetfinder ] || [ -e /usr/local/bin/assetfinder ] || command -v assetfinder &> /dev/null; then  
	...
	if [ -e /usr/local/bin/findomain ] || command -v findomain &> /dev/null; then  
		...
		curl -sL https://github.com/Edu4rdSHL/findomain/releases/latest/download/findomain-linux -o findomain
        
        if [ -f findomain.zip ]; then
            sudo apt-get install -y unzip >/dev/null 2>&1
            unzip -o findomain.zip >/dev/null 2>&1
            chmod +x findomain
            sudo mv findomain /usr/local/bin/findomain
            rm -f findomain.zip
        elif [ -f findomain ]; then
            chmod +x findomain
            sudo mv findomain /usr/local/bin/findomain
        fi
	...
	if [ -e /usr/bin/amass ] || [ -e /usr/local/bin/amass ] || [ -e ~/go/bin/amass ] || command -v amass &> /dev/null; then  
	    echo -e "${BLUE}[!] Amass already exists \n${RESET}"  
	else   
		sudo apt-get install amass -y || go install -v github.com/owasp-amass/amass/v4/...@latest  
...
function installDebian(){  
    sudo apt update -y  
    sudo apt install jq parallel curl wget git unzip -y;  
    echo -e "${RED}[!] Debian Dependencies Installed \n${RESET}"  
    installGo  
    commonToolInstall;  
    echo -e "${BLUE}[!] All Tools Installed Successfully! \n${RESET}"  
    export PATH=$PATH:$HOME/go/bin
}  
...
while [[ "${#}" -gt 0 ]]; do
	...
	case "$( echo ${args} | tr '[:upper:]' '[:lower:]' )" in
		-s|--speed|--small)  
			banner  
			export -f 1crt 2warchive 3dnsbuffer 4threatcrowd 5hackertarget 6certspotter 7anubisdb 8virustotal 9alienvault 10urlscan 11threatminer 12entrust 13riddler 14dnsdumpster 19rapiddns
		...
		-a|--all)  
			banner  
			export -f 1crt 2warchive 3dnsbuffer 4threatcrowd 5hackertarget 6certspotter 7anubisdb 8virustotal 9alienvault 10urlscan 11threatminer 12entrust 13riddler 14dnsdumpster 15findomain 16subfinder 17amass 18assetfinder 19rapiddns 
		...
		-h|--help|*)  
			...
			echo -e "  -s | --small     Small subdomain scan list"  
			echo -e "  -a | --all       All subdomain scan list"  
